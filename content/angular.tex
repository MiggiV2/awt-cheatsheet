...
\section{HTML Basics}
\begin{itemize}[leftmargin=*]
    \item \textbf{Input:} \texttt{<input type="text|password|number" />}
    \item \textbf{Button:} \texttt{<button (click)="...">Click</button>}
    \item \textbf{Select:} \texttt{<select><option>A</option></select>}
    \item \textbf{Attributes:} \texttt{id}, \texttt{class}, \texttt{style}, \texttt{hidden}, \texttt{disabled}
    \item \textbf{Semantic:} \texttt{header}, \texttt{footer}, \texttt{nav}, \texttt{section}, \texttt{article}
\end{itemize}
