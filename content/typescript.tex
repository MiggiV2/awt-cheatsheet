...
\item \textbf{Structural Typing}: Types are compatible if their shape matches (duck typing) [file:4].
\item \textbf{Type Inference}: TS infers types when not explicit (e.g., \texttt{let n = 5} is number) [file:4].
\end{itemize}
...
\item \texttt{unknown}: Safer \texttt{any}; must check type before use [file:4].
...
\begin{itemize}
\item Define object shapes. Can be extended/implemented [file:4].
\end{itemize}
\begin{lstlisting}
interface User {
  id: number;
...
\section{Classes}
\begin{itemize}
\item Access modifiers: \texttt{public} (default), \texttt{private} (\#), \texttt{protected} [file:4].
...
\begin{itemize}
\item Define multiple signatures, then one implementation handling all cases [file:4].
\end{itemize}

% =========================================================
\section{Generics}
\begin{itemize}
\item Reusable components working with various types [file:4].
...
\begin{itemize}
\item Exact values as types [file:4].
\end{itemize}
\begin{lstlisting}
...
\section{Modules}
\begin{itemize}
\item File is a module if it has top-level \texttt{import} or \texttt{export} [file:4].
\end{itemize}
\begin{lstlisting}
// math.ts
...
\begin{itemize}
\item Tell TS a value is definitely not null/undefined [file:4].
\end{itemize}
\begin{lstlisting}
...
\begin{itemize}
\item Stop early if null/undefined [file:4].
\end{itemize}
\begin{lstlisting}
...
\section{Decorators}
\begin{itemize}
\item Functions adding metadata/behavior to classes/members [file:4].
