...
\begin{itemize}
\item \texttt{Array.push(element)}
\item \texttt{JSON.parse(string)}
\item \texttt{JSON.stringify(json)}
\item \texttt{document.getElementById(id)}
\item \texttt{document.getElementsByTagName(name)}
\item \texttt{document.getElementsByClassName(name)}
\end{itemize}
